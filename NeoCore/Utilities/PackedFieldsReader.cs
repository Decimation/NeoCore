...
// 		DWORD   m_rgPackedFields[1];
		...
		private DWORD BitVectorGet(DWORD dwOffset, DWORD dwLength)
		{
			...
			DWORD dwStartBlock = dwOffset / Assets.BITS_PER_DWORD;
			DWORD dwEndBlock   = (dwOffset + dwLength - 1) / Assets.BITS_PER_DWORD;
			...
			if (dwStartBlock == dwEndBlock) {
				...
				DWORD dwValueShift = dwOffset % Assets.BITS_PER_DWORD;
			...
			DWORD dwInitialBits = Assets.BITS_PER_DWORD -
			                      dwOffset % Assets.BITS_PER_DWORD; // Number of bits to get in the first DWORD
			DWORD dwReturn;
